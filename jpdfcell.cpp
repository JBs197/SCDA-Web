...
void JPDFCELL::drawCellText(HPDF_Page& page, JFUNC& jf, string text, HPDF_Font& Font)
{
	...
	while (1) {
		...
		if (error != HPDF_OK) { jf.err("SetFontAndSize-jpdfcell.drawCellText"); }
		spaceWidth = (double)HPDF_Page_TextWidth(page, " ");
		if (spaceWidth == 0.0) { jf.err("TextWidth-jpdfcell.drawCellText"); }
...
void JPDFCELL::drawCellTextList(HPDF_Page& page, JFUNC& jf, int italicFreq, string separator)
{
	...
	while (1) {  
		error = HPDF_Page_SetFontAndSize(page, font, fontSize);
		if (error != HPDF_OK) { jf.err("SetFontAndSize-jpdfcell.drawCellTextList"); }
		sepWidth = (double)HPDF_Page_TextWidth(page, separator.c_str());
		if (sepWidth == 0.0) { jf.err("TextWidth-jpdfcell.drawCellTextList"); }
		if (sepSplit) {
			sepWidthSeg = (double)HPDF_Page_TextWidth(page, sepSegFirst.c_str());
			if (sepWidthSeg == 0.0) { jf.err("TextWidth-jpdfcell.measureCellTextList"); }
		...
		for (int ii = 0; ii < vvsText.size(); ii++) {
			...
			for (int jj = 0; jj < vvsText[ii].size(); jj++) {
				...
				if (textWidth == 0.0) { jf.err("TextWidth-jpdfcell.drawCellTextList"); }
...
void JPDFCELL::drawText(HPDF_Page& page, JFUNC& jf, string text)
{
	...
	if (error != HPDF_OK) { jf.err("BeginText-jpdfcell.drawText"); }
	...
	if (error != HPDF_OK) { jf.err("TextOut-jpdfcell.drawText"); }
	error = HPDF_Page_EndText(page);
	if (error != HPDF_OK) { jf.err("EndText-jpdfcell.drawText"); }
}
