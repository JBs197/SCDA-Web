...
void JPDFMAP::drawCircle(vector<double> coordCenter, double radius, vector<double> colour, double thickness)
{
	...
	if (error != HPDF_OK) { jf.err("SetLineWidth-jpdfmap.drawCircle"); }
	error = HPDF_Page_SetRGBStroke(page, colour[0], colour[1], colour[2]);
	if (error != HPDF_OK) { jf.err("SetRGBStroke-jpdfmap.drawCircle"); }
	error = HPDF_Page_MoveTo(page, coordCenter[0], coordCenter[1]);
	if (error != HPDF_OK) { jf.err("MoveTo-jpdfmap.drawCircle"); }
	error = HPDF_Page_Circle(page, coordCenter[0], coordCenter[1], radius);
	if (error != HPDF_OK) { jf.err("Circle-jpdfmap.drawCircle"); }
	error = HPDF_Page_Stroke(page);
	if (error != HPDF_OK) { jf.err("Stroke-jpdfmap.drawCircle"); }
}
void JPDFMAP::drawCircle(vector<double> coordCenter, double radius, vector<vector<double>> vColour, double thickness)
{
	...
	if (error != HPDF_OK) { jf.err("SetRGBFill-jpdfmap.drawCircle"); }
	error = HPDF_Page_MoveTo(page, coordCenter[0], coordCenter[1]);
	if (error != HPDF_OK) { jf.err("MoveTo-jpdfmap.drawCircle"); }
	error = HPDF_Page_Circle(page, coordCenter[0], coordCenter[1], radius);
	if (error != HPDF_OK) { jf.err("Circle-jpdfmap.drawCircle"); }
	error = HPDF_Page_FillStroke(page);
	if (error != HPDF_OK) { jf.err("FillStroke-jpdfmap.drawCircle"); }
}
...
{
	if (vColour.size() != vColourPos.size()) { jf.err("Size mismatch-jpdfmap.drawGradientH"); }
	int colourIndex = -1;
	...
	if (error != HPDF_OK) { jf.err("SetLineWidth-jpdfmap.drawGradientH"); }
	...
	while (xPos != xStop)
	{
		...
		if (error != HPDF_OK) { jf.err("SetRGBStroke-jpdfmap.drawGradientH"); }
		error = HPDF_Page_MoveTo(page, (float)xPos, heightStartStop[0]);
		if (error != HPDF_OK) { jf.err("MoveTo-jpdfmap.drawGradientH"); }
		error = HPDF_Page_LineTo(page, (float)xPos, heightStartStop[1]);
		if (error != HPDF_OK) { jf.err("MoveTo-jpdfmap.drawGradientH"); }
		error = HPDF_Page_Stroke(page);
		if (error != HPDF_OK) { jf.err("Stroke-jpdfmap.drawGradientH"); }
...
{
	if (vColour.size() != vColourPos.size()) { jf.err("Size mismatch-jpdfmap.drawGradientV"); }
	int colourIndex = -1;
	...
	float b = (float)vColour[0][2];
	HPDF_STATUS error = HPDF_Page_SetLineWidth(page, 1.0);
	if (error != HPDF_OK) { jf.err("SetLineWidth-jpdfmap.drawGradientV"); }
	...
	while (yPos != yStop)
	{
		...
		error = HPDF_Page_SetRGBStroke(page, r, g, b);
		if (error != HPDF_OK) { jf.err("SetRGBStroke-jpdfmap.drawGradientV"); }
		error = HPDF_Page_MoveTo(page, widthStartStop[0], (float)yPos);
		if (error != HPDF_OK) { jf.err("MoveTo-jpdfmap.drawGradientV"); }
		error = HPDF_Page_LineTo(page, widthStartStop[1], (float)yPos);
		if (error != HPDF_OK) { jf.err("MoveTo-jpdfmap.drawGradientV"); }
		error = HPDF_Page_Stroke(page);
		if (error != HPDF_OK) { jf.err("Stroke-jpdfmap.drawGradientV"); }
...
{
	HPDF_STATUS error = HPDF_Page_SetLineWidth(page, thickness);
	if (error != HPDF_OK) { jf.err("SetLineWidth-jpdfmap.drawLine"); }
	error = HPDF_Page_SetRGBStroke(page, colour[0], colour[1], colour[2]);
	if (error != HPDF_OK) { jf.err("SetRGBStroke-jpdfmap.drawLine"); }
	error = HPDF_Page_MoveTo(page, startStop[0][0], startStop[0][1]);
	if (error != HPDF_OK) { jf.err("MoveTo-jpdfmap.drawLine"); }
	error = HPDF_Page_LineTo(page, startStop[1][0], startStop[1][1]);
	if (error != HPDF_OK) { jf.err("LineTo-jpdfmap.drawLine"); }
	error = HPDF_Page_Stroke(page);
	if (error != HPDF_OK) { jf.err("Stroke-jpdfmap.drawLine"); }
}
...
void JPDFMAP::drawRect(vector<vector<double>> rectBLTR, vector<double> colour, double thickness)
{
	// This variant will only draw the perimeter.
	HPDF_STATUS error = HPDF_Page_SetLineWidth(page, thickness);
	if (error != HPDF_OK) { jf.err("SetLineWidth-jpdfmap.drawRect"); }
	error = HPDF_Page_SetRGBStroke(page, colour[0], colour[1], colour[2]);
	if (error != HPDF_OK) { jf.err("SetRGBStroke-jpdfmap.drawRect"); }
	error = HPDF_Page_Rectangle(page, rectBLTR[0][0], rectBLTR[0][1], rectBLTR[1][0] - rectBLTR[0][0], rectBLTR[1][1] - rectBLTR[0][1]);
	if (error != HPDF_OK) { jf.err("Rectangle-jpdfmap.drawRect"); }
	error = HPDF_Page_Stroke(page);
	if (error != HPDF_OK) { jf.err("Stroke-jpdfmap.drawRect"); }
}
void JPDFMAP::drawRect(vector<vector<double>> rectBLTR, vector<vector<double>> vColour, double thickness)
{
	...
	if (error != HPDF_OK) { jf.err("SetRGBStroke-jpdfmap.drawRect"); }
	error = HPDF_Page_SetRGBFill(page, vColour[1][0], vColour[1][1], vColour[1][2]);
	if (error != HPDF_OK) { jf.err("SetRGBFill-jpdfmap.drawRect"); }
	error = HPDF_Page_Rectangle(page, rectBLTR[0][0], rectBLTR[0][1], rectBLTR[1][0] - rectBLTR[0][0], rectBLTR[1][1] - rectBLTR[0][1]);
	if (error != HPDF_OK) { jf.err("Rectangle-jpdfmap.drawRect"); }
	error = HPDF_Page_FillStroke(page);
...
{
	if (PPKM <= 0.0) { jf.err("No PPKM-jpdfmap.getChildTL"); }
	if (childFrameKM.size() != 2) { jf.err("Missing frameKM-jpdfmap.getChildTL"); }
...
{
	if (PPKM <= 0.0) { jf.err("Missing PPKM-jpdfmap.scaleChildToPage"); }
	vvdFrame[0][0] *= PPKM;
	vvdFrame[0][1] *= PPKM;
	vvdFrame[1][0] *= PPKM;
	vvdFrame[1][1] *= PPKM;
...
void JPDFMAP::textBox(vector<vector<double>> boxBLTR, string sText, string alignment, int fontsize)
{
	...
	if (error != HPDF_OK) { jf.err("TextRect-jpdfmap.textBox"); }
