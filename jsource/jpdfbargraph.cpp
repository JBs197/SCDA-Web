...
void JPDFBARGRAPH::drawAxis(vector<vector<double>> startStop, vector<double> vTick, vector<double> colour, double thickness)
{
	...
	else { err("Invalid axis startStop-jpdfbargraph.drawAxis"); }
}
...
void JPDFBARGRAPH::drawAxisY(vector<double>& minMax, string unit)
{
	...
	if (axisTickLines == 2)
	{
		...
		if (textWidth == 0.0) { err("TextWidth-jpdfbargraph.drawAxisY"); }
...
void JPDFBARGRAPH::drawData(vector<vector<double>>& seriesColour)
{
	...
	if (error != HPDF_OK) { err("SetRGBStroke-jpdfbargraph.drawData"); }
	error = HPDF_Page_SetLineWidth(page, 1.0);
	if (error != HPDF_OK) { err("SetLineWidth-jpdfbargraph.drawData"); }
	for (int ii = 0; ii < numRegions; ii++)
	{
		...
		for (int jj = 0; jj < numSeries; jj++)
		{
			...
			if (error != HPDF_OK) { err("SetRGBFill-jpdfbargraph.drawData"); }
			error = HPDF_Page_Rectangle(page, barBL[0], barBL[1], barWidth, barHeight);
			if (error != HPDF_OK) { err("Rectangle-jpdfbargraph.drawData"); }
			error = HPDF_Page_FillStroke(page);
			if (error != HPDF_OK) { err("FillStroke-jpdfbargraph.drawData"); }
			barBL[0] += barWidth;
		}
	}
...
void JPDFBARGRAPH::drawLine(vector<vector<double>> startStop, vector<double> colour, double thickness)
{
	...
	if (error != HPDF_OK) { err("SetLineWidth-jpdf.drawLine"); }
	error = HPDF_Page_SetRGBStroke(page, colour[0], colour[1], colour[2]);
	if (error != HPDF_OK) { err("SetRGBStroke-jpdf.drawLine"); }
	error = HPDF_Page_MoveTo(page, startStop[0][0], startStop[0][1]);
	if (error != HPDF_OK) { err("MoveTo-jpdf.drawLine"); }
	error = HPDF_Page_LineTo(page, startStop[1][0], startStop[1][1]);
	if (error != HPDF_OK) { err("LineTo-jpdf.drawLine"); }
	error = HPDF_Page_Stroke(page);
	if (error != HPDF_OK) { err("Stroke-jpdf.drawLine"); }
}
void JPDFBARGRAPH::err(string message)
{
	string errorMessage = "JPDFBARGRAPH error:\n" + message;
...
void JPDFBARGRAPH::initAxisY(vector<double>& minMax, string unit)
{
	...
	if (error != HPDF_OK) { err("SetRGBFill-jpdfbargraph.initAxisY"); }
	error = HPDF_Page_SetFontAndSize(page, font, fontSize);
	if (error != HPDF_OK) { err("SetFontAndSize-jpdfbargraph.initAxisY"); }
	double textWidthTop = (double)HPDF_Page_TextWidth(page, sTop.c_str());
	if (textWidthTop == 0.0) { err("TextWidth-jpdfbargraph.initAxisY"); }
	if (unit.size() > 1) {
...
vector<string> JPDFBARGRAPH::splitAtSpaceBox(string tooLong, vector<vector<double>>& boxBLTR, double& fontHeight, double thetaRad, double textPadding, double topXMinTR)
{
	...
	while (!success) {
		...
		while (!fit) {
			...
			for (int ii = vsText.size() - 1; ii >= 0; ii--) {
				...
				if (textLength == 0.0) { err("TextWidth-jpdfbargraph.splitAtSpaceBox"); }
	...
	if (textLongest == 0.0) { err("TextWidth-jpdfbargraph.splitAtSpaceBox"); }
...
vector<string> JPDFBARGRAPH::splitAtSpaceEven(string tooLong, int pieces)
{
	...
	if (count < pieces - 1) { err("Insufficient number of spaces-jpdfbargraph.splitAtSpace"); }
...
void JPDFBARGRAPH::textBox(vector<vector<double>> boxBLTR, string sText, string alignment, double fontsize)
{
	...
	if (error != HPDF_OK) { err("SetFontAndSize-jpdfbargraph.textBox"); }
	double textLength = (double)HPDF_Page_TextWidth(page, sText.c_str());
	if (textLength == 0.0) { err("TextWidth-jpdfbargraph.textBox"); }
	...
	if (error != HPDF_OK) { err("BeginText-jpdfbargraph.textBox"); }
	if (alignment == "right") {
	...
	if (error != HPDF_OK) { err("TextRect-jpdfbargraph.textBox"); }
	error = HPDF_Page_EndText(page);
	if (error != HPDF_OK) { err("EndText-jpdfbargraph.textBox"); }
}
void JPDFBARGRAPH::textBoxAngled(string sText, vector<double> corner, string sCorner, double fontsize, double thetaRad)
{
	...
	HPDF_STATUS error = HPDF_Page_SetFontAndSize(page, font, fontsize);
	if (error != HPDF_OK) { err("SetFontAndSize-jpdfbargraph.textBoxAngled"); }
	vector<double> BL;
	if (sCorner == "TR") {
		double textLength = (double)HPDF_Page_TextWidth(page, sText.c_str());
		if (textLength == 0.0) { err("TextWidth-jpdfbargraph.textBoxAngled"); }
		BL = angledTRtoBL(corner[0], corner[1], textLength, fontsize, thetaRad);
	}
	else { BL = corner; }
	error = HPDF_Page_BeginText(page);
	if (error != HPDF_OK) { err("BeginText-jpdfbargraph.textBoxAngled"); }
	...
	if (error != HPDF_OK) { err("SetTextMatrix-jpdfbargraph.textBoxAngled"); }
	error = HPDF_Page_ShowText(page, sText.c_str());
	if (error != HPDF_OK) { err("ShowText-jpdfbargraph.textBoxAngled"); }
	error = HPDF_Page_EndText(page);
	if (error != HPDF_OK) { err("EndText-jpdfbargraph.textBoxAngled"); }
}
