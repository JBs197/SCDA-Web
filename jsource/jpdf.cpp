...
int JPDF::addTable(int numCol, vector<int> vNumLine, vector<double> vRowHeight, string title, double fontSizeTitle)
{
	...
	if (vNumLine.size() != vRowHeight.size()) { err("Size mismatch-jpdf.addTable"); }
	int index = vSection.size();
...
void JPDF::err(string message)
{
	string errorMessage = "JPDF error:\n" + message;
...
vector<double> JPDF::getPageDimensions()
{
	...
	if (vdDim[0] <= 0.0) { err("GetWidth-jpdf.getPageDimensions"); }
	vdDim[1] = (double)(HPDF_Page_GetHeight(page) - (2.0 * margin));
	if (vdDim[1] <= 0.0) { err("GetHeight-jpdf.getPageDimensions"); }
	return vdDim;
}
HPDF_UINT32 JPDF::getPDF(string& sPDF)
{
	...
	if (error != HPDF_OK) { err("SaveToStream-jpdf.getPDF"); }
	HPDF_UINT32 streamSize = HPDF_GetStreamSize(pdf);
	if (!streamSize) { err("GetStreamSize-jpdf.getPDF"); }
	auto buffer = new HPDF_BYTE[streamSize];
	error = HPDF_ReadFromStream(pdf, buffer, &streamSize);
	if (error != HPDF_OK) { err("ReadFromStream-jpdf.getPDF"); }
	...
	if (error != HPDF_OK) { err("ResetStream-jpdf.getPDF"); }
	return streamSize;
}
...
int JPDF::getNumLines(vector<string>& vsText, double width, int iFontHeight)
{
	...
	if (error != HPDF_OK) { err("SetFontAndSize-jpdf.getNumLines"); }
	float separatorWidth = HPDF_Page_TextWidth(page, " | ");
	if (separatorWidth == 0.0) { err("TextWidth-jpdf.getNumLines"); }
	float separatorWidthShort = HPDF_Page_TextWidth(page, " |");
	if (separatorWidthShort == 0.0) { err("TextWidth-jpdf.getNumLines"); }
	...
	for (int ii = 0; ii < vsText.size(); ii++)
	{
		...
		if (textWidth == 0.0) { err("TextWidth-jpdf.getNumLines"); }
		if (remains < textWidth)
		{
			...
			while (remains < textWidth)
			{
				...
				if (textWidth == 0.0) { err("TextWidth-jpdfcell.jpdf.getNumLines"); }
			}
...
void JPDF::init()
{
	...
	if (!pdf) { err("Failed to create pdf object-jpdf.init"); }
	...
	if (error != HPDF_OK) { err("Page_SetSize-jpdf.init"); }
...
void JPDF::textBox(vector<vector<double>> boxBLTR, string sText, string alignment, int fontsize)
{
	...
	if (error != HPDF_OK) { err("SetFontAndSize-jpdf.textBox"); }
	...
	if (error != HPDF_OK) { err("TextRect-jpdf.textBox"); }
